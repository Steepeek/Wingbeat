...
- **Добыча за сессию** строкой внизу: опыт, AP, кинах, убитые мобы,
  ...
  группы что подобрал и какие были броски кубика.
- **Периодический урон приписывается автору.** Тик дота в логе автора не
  содержит, но его можно взять с момента наложения — см. ниже.
- **Звуковые сигналы**: враг ударил вас или согруппника, открылся рифт,
  своя смерть, плюс свои строки.
...
- **Названия предметов недоступны.** В логе предмет записан только
  идентификатором: `You have acquired [item:167000522;ver6;;;;]`. Таблица
  «идентификатор → название» лежит в `Data/Items/items.pak`, зашифрованном
  на приватных серверах. В незашифрованной части клиента этих номеров нет
  вовсе — проверено поиском по всему паку. Поэтому окно добычи показывает
  кто и сколько взял, но предмет — номером.
- **События «враг рядом» в логе нет.** Игра не пишет появление игрока чужой
  фракции; слова «Enemy» встречаются там только как ники. Сигнал даётся по
  тому, что есть: враг уже ударил, либо открылся рифт.
