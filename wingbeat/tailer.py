...
class Tailer:
    ...
    #: Потолок одной порции чтения. Четыре мегабайта — это около 40 тысяч
    #: строк, то есть больше, чем игра пишет между тиками даже в осаде,
    #: и при этом окно не замирает на подменённом файле в сотни мегабайт.
    MAX_CHUNK = 4 * 1024 * 1024

    #: Сколько дочитывать у файла ПОСЛЕ ротации. Игра, создав новый лог,
    #: пишет в него текущую сессию — её начало терять нельзя. Но подмена
    #: файла на чужой лог в сотни мегабайт не должна утягивать в сессию
    #: чужие сутки боёв, поэтому берём только хвост.
    ROTATE_BACKFILL = 4 * 1024 * 1024
    ...
    def read(self) -> list[bytes]:
        ...
        if size < pos or self._rotated():
            # Ротация большого файла не должна читаться с самого начала:
            # интересен хвост, а не весь чужой лог целиком.
            # Лог обрезали, пересоздали или подменили. Начинаем сначала.
            self.rotations += 1
            # Не с начала файла, а с хвоста: при подмене на большой лог
            # чтение с нуля засчитывало в сессию чужие сутки боёв и
            # подвешивало окно на сорок секунд. Для свежесозданного лога
            # игры это ничего не меняет — он меньше порога, и читается весь.
            self.from_start = False
            self.backfill = max(self.backfill, self.ROTATE_BACKFILL)
            try:
                self.open()
            except OSError:
                self.close()
        ...
        # Читаем не больше порции за раз. Иначе подмена лога на большой
        # файл (восстановление из копии, чужой Chat.log, ротация лаунчером)
        # проглатывалась одним куском: замер показал 45 секунд без единого
        # обновления окна и рост памяти со 177 до 796 МБ. Остаток дочитается
        # следующими тиками, они идут четыре раза в секунду.
        chunk = self.f.read(min(size - pos, self.MAX_CHUNK))
        if not chunk:
            return []
